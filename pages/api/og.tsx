...
export default async function handler(req: NextRequest) {
    try {
        const fontMonaSans = await monaSans;
        const fontHubotSans = await hubotSans;

        const { searchParams } = new URL(req.url);

        const title = searchParams.get('title')?.slice(0, 100) || "Hello world";
        const pageType = searchParams.get('pagetype')?.slice(0, 100) || 'A page';
            ...
            (
                <div tw="bg-neutral-900 p-12 h-full w-full flex items-end justify-between flex-col flex-nowrap"
                    ...
                    <div id="main" tw="grow flex items-center justify-center w-full"
                        style={{ fontFamily: 'HubotSans', filter: 'drop-shadow(0px 10px 8px #00000014) drop-shadow(0 4px 3px #00000033)' }}>
